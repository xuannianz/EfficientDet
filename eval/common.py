...
def _get_detections(generator, model, score_threshold=0.05, max_detections=100, visualize=False):
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        # run network
        boxes, scores, _, _, labels = model.predict_on_batch([np.expand_dims(image, axis=0),
        ...
        for i in range(generator.size()):
            ...
            for d in detections:
                ...
                if annotations.shape[0] == 0:
                    ...
                    continue
                overlaps = compute_overlap(np.expand_dims(d, axis=0).astype(np.float32), annotations.astype(np.float32))
